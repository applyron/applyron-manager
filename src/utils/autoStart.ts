...
import { MANAGER_PRODUCT_NAME, MANAGER_PRODUCT_SLUG } from '../config/managerBrand';

export const AUTO_START_ARG = '--autostart';
const LINUX_AUTOSTART_FILENAME = `${MANAGER_PRODUCT_SLUG}.desktop`;
...
function cleanupLinuxAutoStartEntries(currentPath: string) {
  ...
  const legacyNames = [
    ...
    `${MANAGER_PRODUCT_NAME}.desktop`,
    `${MANAGER_PRODUCT_NAME.replace(/\s+/g, '')}.desktop`,
