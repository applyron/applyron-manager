...
function getEarliestResetTime(models: ModelQuota[]): string | null {
  const resetTimes = map(models, (model) => parseResetTimestamp(model.resetTime)).filter(
    (timestamp): timestamp is number => timestamp !== null,
  );

  const earliestTimestamp = min(resetTimes);
  return earliestTimestamp !== undefined ? new Date(earliestTimestamp).toISOString() : null;
}
...
export function calculateProviderStats(
  ...
  return buildProviderStats(
    ...
    models,
    visibleModels,
    getEarliestResetTime(visibleModels),
  );
}
...
export function groupVisibleModelsByProvider(models: ModelQuota[]): AccountStats {
  const providerModelGroups = groupBy(models, (model) => detectProvider(model.id));
  const providerStatsList = map(providerModelGroups, (groupedModels, key) => {
    const providerKey = key as ProviderKey;
    return buildProviderStats(
      providerKey,
      groupedModels,
      groupedModels,
      getEarliestResetTime(groupedModels),
    );
  });
  ...
  return {
    ...
    overallPercentage,
    healthStatus: resolveAccountHealthStatus(overallPercentage),
  };
}
