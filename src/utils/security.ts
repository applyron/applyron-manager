...
import {
  getManagerKeychainServiceNames,
  getManagerUserDataDirCandidates,
  MANAGER_KEYCHAIN_SERVICE_NAME,
} from '../config/managerBrand';
import { ServiceHealthRegistry } from '../services/ServiceHealthRegistry';

const SERVICE_NAME = MANAGER_KEYCHAIN_SERVICE_NAME;
...
const DEGRADED_SECURITY_MESSAGE = 'Degraded security: using file-based key storage.';
const HEALTHY_SECURITY_MESSAGE = 'Credential storage is backed by OS-protected storage.';

export type KeySource = 'safeStorage' | 'keytar' | 'file';

interface DecryptWithMigrationOptions {
  suppressAuthTagMismatchLog?: boolean;
}
...
function getLegacyFallbackKeyPaths(): string[] {
  return dedupe(
    getManagerUserDataDirCandidates()
      .slice(1)
      .map((dirPath) => path.join(dirPath, '.mk')),
  );
}
...
async function readKeytarKey(serviceName = SERVICE_NAME): Promise<Buffer | null> {
  const keytar = await tryKeytar();
  if (!keytar) {
    return null;
  }

  const existingKey = await keytar.getPassword(serviceName, ACCOUNT_NAME);
  if (!existingKey) {
    return null;
  }
...
async function getFallbackMasterKeys(
  ...
  const fileKeyPaths = dedupe([keyPath, ...getLegacyFallbackKeyPaths()]);

  if (primarySource !== 'keytar') {
    for (const serviceName of getManagerKeychainServiceNames()) {
      const keytarKey = await readKeytarKey(serviceName);
      if (keytarKey) {
  ...
  if (primarySource !== 'file') {
    for (const filePath of fileKeyPaths) {
      const fileKey = await readFileFallbackKey(filePath);
      if (fileKey) {
  ...
  if (primarySource !== 'safeStorage') {
    for (const filePath of fileKeyPaths) {
      const safeStorageKey = await readSafeStorageKey(filePath);
      if (safeStorageKey) {
...
async function generatePrimaryMasterKey(): Promise<MasterKeyState> {
  ...
  if (safeStorage.isEncryptionAvailable()) {
    try {
      ...
      ServiceHealthRegistry.markReady('security', HEALTHY_SECURITY_MESSAGE);
  ...
  try {
    ...
    ServiceHealthRegistry.markError('security', DEGRADED_SECURITY_MESSAGE);
    ...
    ServiceHealthRegistry.markError('security', 'Credential storage is unavailable.');
...
export async function decryptWithMigration(
  text: string,
  options?: DecryptWithMigrationOptions,
  ...
  try {
    ...
    if (!options?.suppressAuthTagMismatchLog) {
      logger.error(
...
export async function decrypt(
  ...
  const result = await decryptWithMigration(text, options);
  return result.value;
}
