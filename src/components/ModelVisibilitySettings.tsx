...
import { filter, includes, size } from 'lodash-es';
...
export function ModelVisibilitySettings() {
  ...
  const visibilityStats = useMemo(
    () => summarizeModelVisibility(availableModelIds, modelVisibilityMap),
    [availableModelIds, modelVisibilityMap],
  );
      ...
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge variant="secondary">
            {searchQuery.trim()
              ? t('settings.modelVisibility.resultsCount', { count: filteredModelIds.length })
              : t('settings.modelVisibility.modelsShown', {
                  visible: visibilityStats.visibleCount,
                  total: visibilityStats.totalCount,
                })}
          </Badge>
        </CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-muted-foreground border-t pt-2 text-sm">
          <div className="flex justify-between">
            <span>
              {t('settings.modelVisibility.totalModels')}: {visibilityStats.totalCount}
            </span>
            <span>
              {t('settings.modelVisibility.visibleModels')}: {visibilityStats.visibleCount}
            </span>
            <span>
              {t('settings.modelVisibility.hiddenModels')}: {visibilityStats.hiddenCount}
            </span>
          </div>
        </div>
      </CardContent>
