...
export interface RateLimitTrackerSummary {
...
export class RateLimitTracker {
  ...
  getSummary(): RateLimitTrackerSummary {
    this.cleanupExpired();

    let nextRetryAt: number | null = null;
    let nextRetrySec: number | null = null;
    const reasonSummary: Record<string, number> = {};

    for (const info of this.lockoutByKey.values()) {
      reasonSummary[info.reason] = (reasonSummary[info.reason] ?? 0) + 1;

      if (nextRetryAt === null || info.resetTimeMs < nextRetryAt) {
        nextRetryAt = info.resetTimeMs;
        nextRetrySec = info.retryAfterSec;
      }
    }

    return {
      activeLockCount: this.lockoutByKey.size,
      reasonSummary,
      nextRetryAt,
      nextRetrySec,
    };
