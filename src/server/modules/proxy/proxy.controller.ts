...
export class ProxyController {
  ...
  constructor(
    ...
    @Optional()
    @Inject(ProxyMetricsRegistry)
    private readonly metricsRegistry?: ProxyMetricsRegistry,
  ) {}
  ...
  async completions(
    ...
    try {
      ...
      this.recordProxySuccess(request.model, startedAt, Boolean(body.stream));
    } catch (error) {
      this.sendOpenAIErrorResponse(res, '/v1/completions', error, undefined, {
        model: request.model,
        startedAt,
      });
    }
  ...
  async responses(
    ...
    try {
      ...
      this.sendOpenAIErrorResponse(res, '/v1/responses', error, undefined, {
        model: request.model,
        startedAt,
      });
    }
  ...
  async imageGenerations(
    ...
    await this.sendOpenAIImageGenerationResponse(request, body.prompt ?? '', res, startedAt);
  }
  ...
  async audioTranscriptions(
    ...
    try {
      ...
      this.recordProxySuccess(body.model, startedAt);
    } catch (error) {
      this.sendOpenAIErrorResponse(res, '/v1/audio/transcriptions', error, undefined, {
        model: body.model,
        startedAt,
      });
    }
  ...
  private async respondOpenAIChatCompletions(body: OpenAIChatRequest, res: FastifyReply) {
    ...
    try {
      ...
      if (body.stream && this.isObservableLike(result)) {
        ...
        this.recordProxySuccess(body.model, startedAt, body.stream);
      }
    } catch (error) {
      this.sendOpenAIErrorResponse(res, '/v1/chat/completions', error, undefined, {
        model: body.model,
        startedAt,
      });
    }
  ...
  async anthropicMessages(@Body() body: AnthropicChatRequest, @Res() res: FastifyReply) {
    ...
    try {
      ...
      if (body.stream && this.isObservableLike(result)) {
        ...
        res.status(HttpStatus.OK).send(result);
        this.recordProxySuccess(body.model, startedAt, Boolean(body.stream));
      }
    } catch (error) {
      this.sendAnthropicErrorResponse(res, '/v1/messages', error, undefined, {
        model: body.model,
        startedAt,
      });
    }
  ...
  private resolveInlineData(input: unknown): {
    ...
    if (isString(input)) {
      const dataUri = this.parseBase64DataUri(input);
      if (dataUri) {
        return dataUri;
      }

      const cleaned = /\s/.test(input) ? input.replace(/\s+/g, '') : input;
      if (cleaned.length > 0) {
        return {
          mimeType: 'audio/mpeg',
          data: cleaned,
  ...
  private parseBase64DataUri(input: string): { mimeType: string; data: string } | null {
    ...
    const mimeType = input.slice(5, markerIndex).trim();
    const data = input.slice(markerIndex + base64Marker.length).trim();
    if (!mimeType || !data) {
      return null;
    }
  ...
  private writeSseResponse(
    ...
    const subscription = stream.subscribe({
      ...
      complete: () => {
        if (!finished && metrics) {
          finished = true;
          this.recordProxySuccess(metrics.model, metrics.startedAt, true);
        }
        if (!res.raw.writableEnded) {
          res.raw.end();
        }
  ...
  private async sendOpenAIImageGenerationResponse(
    ...
    try {
      ...
      this.recordProxySuccess(request.model, startedAt);
    } catch (error) {
  ...
  private sendOpenAIErrorResponse(
    ...
    if (error instanceof ProxyCapacityError) {
      this.logProxyEndpointError(endpoint, HttpStatus.SERVICE_UNAVAILABLE, error.message, error);
      if (metrics) {
        this.recordProxyError(
          metrics.model,
          metrics.startedAt,
      ...
      res.header('Retry-After', String(error.retryAfterSec));
  ...
  private recordProxySuccess(model: string | undefined, startedAt: number, isStream = false): void {
    this.metricsRegistry?.recordRequest({
      model,
  ...
  private recordProxyError(
    model: string | undefined,
    startedAt: number,
    status: number,
    error: unknown,
    isCapacityReject = false,
    ...
    this.metricsRegistry?.recordRequest({
      ...
      isRateLimitEvent,
      errorMessage:
        error instanceof Error ? error.message : `Proxy request failed with status ${status}.`,
    });
  }
}
