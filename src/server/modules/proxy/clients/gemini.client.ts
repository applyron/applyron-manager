...
export class GeminiClient {
  ...
  private getInternalTimeoutMs(configSnapshot?: Readonly<ProxyConfig> | null): number {
  ...
  private async executeRequestWithEndpointFailover<T>(
    ...
    const timeout = this.getInternalTimeoutMs(configSnapshot);
    const requestUserAgent = await resolveRequestUserAgent();
    const axiosProxy = this.resolveUpstreamAxiosProxy(upstreamProxyUrl, configSnapshot);
