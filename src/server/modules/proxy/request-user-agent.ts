...
const DEFAULT_REMOTE_VERSION_URL =
  'https://antigravity-auto-updater-974169037036.us-central1.run.app';
const DEFAULT_CHANGELOG_URL = 'https://antigravity.google/changelog';
...
export interface RequestUserAgentConfig {
...
export function resolveRequestUserAgentConfig(): RequestUserAgentConfig {
  return {
    userAgentOverride: normalizeNonEmptyString(process.env.APPLYRON_REQUEST_USER_AGENT),
    remoteVersionUrl:
      normalizeNonEmptyString(process.env.APPLYRON_REMOTE_VERSION_URL) ??
      DEFAULT_REMOTE_VERSION_URL,
    changelogUrl:
      normalizeNonEmptyString(process.env.APPLYRON_CHANGELOG_URL) ?? DEFAULT_CHANGELOG_URL,
  };
}
...
async function fetchTextPayload(url: string): Promise<string | null> {
  try {
    const { userAgentOverride } = resolveRequestUserAgentConfig();
    const discoveryVersion = resolveLocalInstalledVersion() ?? FALLBACK_VERSION;
    const response = await axios.get<string>(url, {
      timeout: DEFAULT_REMOTE_TIMEOUT_MS,
      responseType: 'text',
      headers: {
        'User-Agent': userAgentOverride ?? buildUserAgent(discoveryVersion),
      },
    });
...
async function resolveRemoteVersion(): Promise<{
  ...
  const { remoteVersionUrl, changelogUrl } = resolveRequestUserAgentConfig();

  const remotePayload = await fetchTextPayload(remoteVersionUrl);
  if (remotePayload) {
  ...
  const changelogPayload = await fetchTextPayload(changelogUrl);
  if (!changelogPayload) {
    return null;
  }
...
export function resetRequestUserAgentResolutionCache(): void {
  cachedUserAgentResolution = null;
  pendingUserAgentResolution = null;
}
