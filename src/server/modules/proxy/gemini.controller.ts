...
export class GeminiController {
    ...
    try {
      if (action === 'streamGenerateContent') {
        ...
        if (stream instanceof Observable) {
          this.writeObservableSseResponse(res, stream, { model, startedAt });
          return;
        }
      }
      ...
      if (action === 'generateContent') {
        ...
        this.recordRequest(model, startedAt, true);
        return;
      }
      ...
      this.recordRequest(model, startedAt, false, message);
  ...
  private writeObservableSseResponse(
    ...
    metrics?: { model?: string; startedAt: number },
  ): void {
    if (
      !res.raw ||
    ...
    const subscription = stream.subscribe({
      ...
      complete: () => {
        if (!finished && metrics) {
          finished = true;
          this.recordRequest(metrics.model, metrics.startedAt, true, null, true);
        }
        if (!res.raw.writableEnded) {
          res.raw.end();
        }
  ...
  private recordRequest(
    ...
    this.metricsRegistry?.recordRequest({
      ...
      success,
      isStream,
      errorMessage: success ? null : (errorMessage ?? 'Gemini proxy request failed.'),
    });
  }
}
