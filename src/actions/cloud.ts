...
function getE2ECloudAccountsFailure(): Error | null {
  if (typeof window === 'undefined') {
    return null;
  }

  if (window.electronTest?.getOrpcTestMode?.() === 'cloudAccountsFailure') {
...
export function listCloudAccounts() {
  const e2eFailure = getE2ECloudAccountsFailure();
  if (e2eFailure) {
    return Promise.reject(e2eFailure);
  }
...
export function deleteCloudAccountsBatch(input: {
  accountIds: string[];
}): Promise<DeleteCloudAccountsBatchResult> {
  return ipc.client.cloud.deleteCloudAccountsBatch(input);
}
