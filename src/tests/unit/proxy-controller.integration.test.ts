...
import { ProxyCapacityError } from '../../server/modules/proxy/clients/upstream-error';
...
describe('ProxyController Integration', () => {
  ...
  it('returns normalized 503 capacity errors with retry metadata', async () => {
    const proxyService = {
      handleChatCompletions: vi.fn().mockRejectedValue(
        new ProxyCapacityError({
  ...
  it('normalizes raw base64 audio payloads with whitespace', async () => {
    const proxyService = {
      handleGeminiGenerateContent: vi.fn().mockResolvedValue({
        candidates: [
          {
    ...
    const request = proxyService.handleGeminiGenerateContent.mock.calls[0][1];
    const inlineData = request.contents?.[0]?.parts?.[1]?.inlineData;

    expect(inlineData).toEqual({
      mimeType: 'audio/mpeg',
      data: 'QUJDRA==',
    });
    expect(reply.status).toHaveBeenCalledWith(200);
