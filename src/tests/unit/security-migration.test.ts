...
describe('decryptWithMigration', () => {
  ...
  it('can suppress the auth-tag mismatch log for expected background failures', async () => {
    keytarMock.getPassword.mockResolvedValue(null);
    fsMock.readFile.mockImplementation(async (_path, encoding) => {
      if (encoding === 'utf8') {
        return 'not-hex';
    ...
    await expect(
      decryptWithMigration(ciphertext, { suppressAuthTagMismatchLog: true }),
    ).rejects.toThrow('ERR_DATA_MIGRATION_FAILED');
