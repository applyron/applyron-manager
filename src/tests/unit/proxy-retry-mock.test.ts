...
const { mockResolveRequestUserAgent } = vi.hoisted(() => ({
  mockResolveRequestUserAgent: vi.fn(),
}));

vi.mock('../../server/modules/proxy/request-user-agent', async () => {
  const actual = await vi.importActual<
    typeof import('../../server/modules/proxy/request-user-agent')
  >('../../server/modules/proxy/request-user-agent');

  return {
    ...actual,
    resolveRequestUserAgent: mockResolveRequestUserAgent,
  };
});

import { ProxyService } from '../../server/modules/proxy/proxy.service';
import { generateInternalWithStreamFallback } from '../../server/modules/proxy/proxy-generation-helpers';
import {
...
import { resetRequestUserAgentResolutionCache } from '../../server/modules/proxy/request-user-agent';
import {
  passthroughSseStream,
  processAnthropicInternalStream,
  processOpenAICompatibleStream,
} from '../../server/modules/proxy/proxy-stream-helpers';
...
const mockProxyLogger = {
  error: vi.fn(),
  warn: vi.fn(),
};

function createService() {
  return new ProxyService(mockTokenManager as any, mockGeminiClient as any);
}
...
describe('ProxyService Empty Stream Retry Logic', () => {
  beforeEach(() => {
    ...
    mockResolveRequestUserAgent.mockResolvedValue('antigravity/1.20.6 windows/amd64');
  });

  it('classifies retry matrix consistently', () => {
    expect(classifyUpstreamFailure('401 unauthorized token')).toEqual({
      retry: true,
      markAsForbidden: true,
      markAsRateLimited: false,
    });
    expect(classifyUpstreamFailure('403 permission_denied')).toEqual({
      retry: true,
      markAsForbidden: true,
      markAsRateLimited: false,
    });
    expect(classifyUpstreamFailure('429 quota exceeded')).toEqual({
      retry: true,
      markAsForbidden: false,
      markAsRateLimited: true,
    });
    expect(classifyUpstreamFailure('500 internal error')).toEqual({
      retry: true,
      markAsForbidden: false,
      markAsRateLimited: false,
    });
    expect(classifyUpstreamFailure('400 invalid argument')).toEqual({
      retry: false,
      markAsForbidden: false,
      markAsRateLimited: false,
    });
  ...
  it('builds Claude-specific beta headers consistently', () => {
    const claudeHeaders = createModelSpecificHeaders('claude-sonnet-4-5');
    const geminiHeaders = createModelSpecificHeaders('gemini-2.5-flash');
  ...
  it('should emit error when stream ends without data', async () => {
    ...
    const resultObservable = processAnthropicInternalStream(
      asReadableStream(stream),
      mockProxyLogger,
    );
  ...
  it('falls back to stream aggregation when non-stream response is empty', async () => {
    ...
    const promise = generateInternalWithStreamFallback({
      geminiClient: mockGeminiClient as any,
      body: { model: 'gemini-2.5-flash' } as any,
      accessToken: 'token',
  ...
  it('raises error for empty Gemini passthrough stream', async () => {
    ...
    const observable = passthroughSseStream(asReadableStream(stream));
    let errorMessage = '';
  ...
  it('propagates Anthropic stream interruption errors', async () => {
    const stream = new EventEmitter();
    const observable = processAnthropicInternalStream(asReadableStream(stream), mockProxyLogger);
    let errorMessage = '';
...
describe('ProxyService Protocol Parity Fixtures', () => {
  it('maps OpenAI request to Anthropic request with tools and tool result', () => {
    ...
    const anthropicRequest = convertOpenAIToClaude(openaiRequest as any);
    ...
    expect(assistantBlocks.some((block: any) => block.type === 'tool_use')).toBe(true);

    const toolResultMessage = anthropicRequest.messages[2];
    const toolResultBlocks = Array.isArray(toolResultMessage.content)
  ...
  it('maps Anthropic response to OpenAI response with reasoning and tool_calls', () => {
    ...
    const openaiResponse = convertClaudeToOpenAIResponse(anthropicResponse as any, 'gpt-4o-mini');
  ...
  it('converts internal SSE stream into OpenAI SSE chunks', async () => {
    const stream = new EventEmitter();
    const observable = processOpenAICompatibleStream(asReadableStream(stream), 'gpt-4o-mini');
