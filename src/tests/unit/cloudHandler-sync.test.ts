...
describe('CloudAccountRepo.syncFromIDE', () => {
  ...
  it('should prefer new format when capability detection finds unified key', async () => {
    ...
    await RepoWithMock.injectCloudToken({
      id: 'id',
      provider: 'google',
...
describe('cloud switch fail-fast path', () => {
  ...
  it('should fail fast without rollback or forced restart when inject fails', async () => {
    ...
    const recordSwitchRollbackMock = vi.fn();
    ...
    vi.doMock('../../ipc/switchMetrics', () => ({
      ...
      recordSwitchRollback: recordSwitchRollbackMock,
    ...
    await expect(switchCloudAccount('acc-1')).rejects.toThrow(
      'switch_failed|Failed to apply the requested switch.',
    );
    ...
    expect(startAntigravityMock).toHaveBeenCalledTimes(1);
    expect(recordSwitchRollbackMock).toHaveBeenCalledWith('cloud', true);
  ...
  it('waits for async token injection before restarting the app process', async () => {
    vi.resetModules();

    const applyDeviceProfileMock = vi.fn();
    const startAntigravityMock = vi.fn(async () => undefined);
    const updateLastUsedMock = vi.fn();
    const setActiveMock = vi.fn();
    const resolveInjectRef: { current: (() => void) | null } = { current: null };
    const injectCloudTokenMock = vi.fn(
      () =>
        new Promise<void>((resolve) => {
          resolveInjectRef.current = resolve;
        }),
    );

    const account = {
      id: 'acc-2',
      email: 'cloud-async@test.dev',
      name: 'Cloud Async User',
      provider: 'google' as const,
      token: {
        access_token: 'access',
        refresh_token: 'refresh',
    ...
    vi.doMock('../../ipc/database/cloudHandler', () => ({
      CloudAccountRepo: {
        ...
        setDeviceBinding: vi.fn(),
        updateToken: vi.fn(async () => undefined),
        injectCloudToken: injectCloudTokenMock,
    ...
    const switchPromise = switchCloudAccount('acc-2');

    await vi.waitFor(() => {
      expect(injectCloudTokenMock).toHaveBeenCalledTimes(1);
    });
    expect(startAntigravityMock).not.toHaveBeenCalled();
    expect(updateLastUsedMock).not.toHaveBeenCalled();
    expect(setActiveMock).not.toHaveBeenCalled();

    resolveInjectRef.current?.();
    await switchPromise;

    expect(startAntigravityMock).toHaveBeenCalledTimes(1);
    expect(updateLastUsedMock).toHaveBeenCalledWith('acc-2');
    expect(setActiveMock).toHaveBeenCalledWith('acc-2');
  });
});
