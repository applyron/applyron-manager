...
import {
  ...
  addGoogleAccount,
  deleteCloudAccount,
  type DeleteCloudAccountsBatchResult,
...
export function useDeleteCloudAccountsBatch() {
  const queryClient = useQueryClient();

  return useMutation<DeleteCloudAccountsBatchResult, Error, { accountIds: string[] }>({
    mutationFn: deleteCloudAccountsBatch,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.cloudAccounts });
    },
  });
